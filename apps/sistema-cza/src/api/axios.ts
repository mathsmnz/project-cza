...
/**
 * Constructs the URL to access a specific file belonging to a given project.
 *
 * <p>
 * Behavior:
 * <ul>
 *   <li>Builds the full API endpoint URL for the specified file.</li>
 *   <li>Properly encodes the filename to handle special characters.</li>
 *   <li>Returns a string URL that can be used directly in img src, anchor href, etc.</li>
 *   <li>Does not verify file existence - returns URL regardless.</li>
 * </ul>
 * </p>
 *
 * @param projectId The ID of the project the file belongs to.
 ...
 * ```ts
 * // For images
 * const imageUrl = getProjectFileUrl('project123', 'floor-plan.png')
 * <img :src="imageUrl" />
 *
 * // For IFC files (to be loaded as blob separately)
 * const ifcUrl = getProjectFileUrl('project123', 'model.ifc')
 *
 * // For direct download links
 * const pdfUrl = getProjectFileUrl('project123', 'report.pdf')
 * <a :href="pdfUrl" download>Download PDF</a>
 * ```
 *
 * @note For large files (IFC, etc.), consider using downloadProjectFile()
 *       with progress tracking instead of direct URLs.
 */
export const getProjectFileUrl = (
  ...
  return `${baseUrl}/api/files/v1/${projectId}/${encodeURIComponent(filename)}`
}

/**
 * Sends a new telemetry record to the server.
 * Maps to POST /api/telemetry/v1
 *
 * <p>
 * Behavior:
 * <ul>
 * <li>Validates the payload against the backend requirements.</li>
 * <li>Sanitizes input on the server side (XSS protection).</li>
 * <li>Returns the created TelemetryData object with the generated ID.</li>
 * </ul>
 * </p>
 *
 * @param request The telemetry data payload to send.
 * @returns A Promise resolving to the created {@link TelemetryData}.
 *
 * @example
 * ```ts
 ...
 * }
 * try {
 * const result = await createTelemetry(payload)
 ...
 * ```
 *
 * @throws If the request fails (e.g., validation error, network error),
...
export const createTelemetry = async (
  ...
  try {
    const response = await api.post<TelemetryData>('/api/telemetry/v1', request)
    return response.data
  } catch (error) {
    const err = error as AxiosError
    console.error(`Failed to create telemetry record for user ${request.userId}:`, err)
    throw err
  }
}

/**
 * Retrieves all telemetry records from the server.
 * Maps to GET /api/telemetry/v1
 *
 * <p>
 * Behavior:
 * <ul>
 * <li>Fetches the complete list of telemetry entries.</li>
 * <li>Useful for admin dashboards or aggregate analysis.</li>
 * </ul>
 * </p>
 *
 * @returns A Promise resolving to an array of {@link TelemetryData}.
 *
 * @example
 * ```ts
 * const allData = await getAllTelemetry()
...
export const getAllTelemetry = async (): Promise<TelemetryData[]> => {
  try {
    ...
    const err = error as AxiosError
    console.error('Failed to fetch all telemetry records:', err)
    throw err
  }
}

/**
 * Retrieves a specific telemetry record by its unique ID.
 * Maps to GET /api/telemetry/v1/{id}
 *
 * @param id The unique identifier of the telemetry record.
 * @returns A Promise resolving to the {@link TelemetryData}.
 *
 * @example
 * ```ts
 * const record = await getTelemetryById(5)
 * console.log('Session time:', record.elapsedTime)
 * ```
 *
...
export const getTelemetryById = async (id: number): Promise<TelemetryData> => {
  try {
    ...
    const err = error as AxiosError
    console.error(`Failed to fetch telemetry record with ID ${id}:`, err)
    throw err
  }
}

/**
 * Retrieves all telemetry records associated with a specific user.
 * Maps to GET /api/telemetry/v1/user/{userId}
 *
 * <p>
 * Behavior:
 * <ul>
 * <li>Filters records server-side by the exact user ID string.</li>
 * <li>Properly encodes the userId to handle special characters.</li>
 * </ul>
 * </p>
 *
 * @param userId The unique string identifier of the user.
 * @returns A Promise resolving to an array of {@link TelemetryData} for that user.
 *
 * @example
 * ```ts
 * const userRecords = await getTelemetryByUserId('user-uuid-123')
 * const totalSubmissions = userRecords.reduce((acc, curr) => acc + curr.formSubmissions, 0)
 * ```
 */
export const getTelemetryByUserId = async (
  userId: string,
): Promise<TelemetryData[]> => {
  try {
    const encodedUserId = encodeURIComponent(userId)
    const response = await api.get<TelemetryData[]>(
      `/api/telemetry/v1/user/${encodedUserId}`,
    )
    return response.data
  } catch (error) {
    const err = error as AxiosError
    console.error(`Failed to fetch telemetry for user ${userId}:`, err)
    throw err
  }
}
