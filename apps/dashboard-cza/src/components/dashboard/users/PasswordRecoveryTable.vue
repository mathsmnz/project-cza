...
<template>
  <div class="overflow-x-auto">
    <table class="w-full text-sm text-left text-gray-600">
      ...
      <tr v-for="request in requests" :key="request.email" class="border-b hover:bg-gray-50">
        <td class="p-4">
          <input
            type="checkbox"
            class="w-4 h-4 text-black bg-gray-100 border-gray-300 focus:ring-black"
        ...
        <td class="py-4 px-6">{{ request.status }}</td>
        <td class="py-4 px-6 text-right">
          <div class="flex justify-end items-center space-x-2">
            <button class="text-gray-500 hover:text-black" @click="emit('refresh', request.email)">
              <!-- edit icon -->
              <svg
            ...
            <button
              ...
              @click="emit('delete', request.status)"
            >
              <!-- trash icon -->
              <svg
                xmlns="http://www.w3.org/2000/svg"
