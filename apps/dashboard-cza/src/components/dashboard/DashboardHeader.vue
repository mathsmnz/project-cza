...
const emit = defineEmits(['createProject', 'createUser', 'search', 'toggleSidebar'])
...
<template>
  ...
  >
    <div class="flex items-center justify-between max-w-7xl mx-auto gap-4">
      <!-- Left section: Hamburger (mobile only) + Action button -->
      <div class="flex items-center gap-2 sm:gap-4 min-w-0 flex-shrink-0">
        <!-- Hamburger Button -->
        <button
          @click="emit('toggleSidebar')"
          class="md:hidden p-2 text-black border-2 border-transparent hover:border-black transition-colors"
